...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train = x_train.reshape(num_train, -1)  # flatten to treat as vectors
    x_test = x_test.reshape(num_test, -1)

    for i in range(num_train):
        for j in range(num_test):
            diff = x_train[i] - x_test[j]
            dist = torch.sqrt(torch.matmul(diff.t(), diff))  # L2 (Euclidean distance)
            dists[i, j] = dist
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        diff = x_train[i] - x_test  # between all train image and the first test image 
        dist = torch.pow(torch.sum(torch.pow(diff, 2), dim=1), 0.5)  # L2 (Euclidean distance)
        dists[i, :] = dist
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    dists = torch.sqrt(torch.sum(torch.pow(x_train, 2), dim=1, keepdim=True) 
                       + torch.sum(torch.pow(x_test, 2), dim=1, keepdim=True).t()
                       - 2*torch.matmul(x_train, x_test.t()))

    ## I got hint from the Lecture note No.3 (03 math for programming)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        ...
        output, counts = torch.unique(y_train[indices], sorted=True, return_counts=True)
        y_pred[i] = min(output[counts == max(counts)])
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for fold in range(num_folds):
            x_test_val = x_train_folds[fold]
            y_test_val = y_train_folds[fold]
            x_train_val = torch.cat([*x_train_folds[:fold], *x_train_folds[fold+1:]])  # use decorator for torch.cat since input is tuple
            y_train_val = torch.cat([*y_train_folds[:fold], *y_train_folds[fold+1:]])
            
            classifier = KnnClassifier(x_train_val, y_train_val)
            accuracy = classifier.check_accuracy(x_test_val, y_test_val, k)
