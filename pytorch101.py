...
def count_tensor_elements(x: Tensor) -> int:
    ...
    ##########################################################################
    # Replace "pass" with your code (do not modify this line)
    num_elements = torch.flatten(x).size(dim=0)
    # flatten; reshape into a one-dim tensor
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" with your code (do not modify this line)
    numlist = torch.arange(start, stop+1, 1).to(torch.float64)
    x = numlist[numlist % 10 == 0]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def shuffle_cols(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" with your code (do not modify this line)
    y = torch.zeros(size=(x.shape[0],4))
    y[:, :2] = x[:, :1]
    y[:, 2] = x[:, 2]
    y[:, 3] = x[:, 1]
    ##########################################################################
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" with your code (do not modify this line)
    y = x.reshape(-1, 3, 4).transpose(0, 1).reshape(3, -1)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" with your code (do not modify this line)
    y = torch.clone(x)
    _, row_min_idxs = torch.min(y, dim=1)  # reduction
    row_all_idxs = torch.arange(y.shape[0])
    y[row_all_idxs, row_min_idxs] = torch.tensor([0])
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    ###########################################################################
    # Replace "pass" with your code (do not modify this line)
    # z = torch.matmul(x[0], y[0])
    # B = x.shape[0]
    # for i in range(1, B):
    #     temp = torch.matmul(x[i], y[i])
    #     z = torch.stack([z, temp], dim=0)
    # z.to(x)
    z = torch.matmul(x[0], y[0]).flatten()
    B = x.shape[0]
    for i in range(1, B):
        temp = torch.matmul(x[i], y[i]).flatten()
        z = torch.cat([z, temp])
    z = z.reshape(B, x.shape[1], y.shape[2])
    z.to(x)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    mean_vector = torch.matmul(ones, x)/3
    subtraction = x - mean_vector  # broadcasting
    cov_matrix = torch.sqrt(torch.matmul(subtraction.t(), subtraction)/(3-1))
    sigma_vector = torch.diag(cov_matrix)

    y = (x - mean_vector)/sigma_vector
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
